...
#from modules.chain import get_qa_chain
# deeplake is not used (for the moment, maybe later)
# from datachad.backend.deeplake import (
#     get_data_source_from_deeplake_dataset_path,
#     get_deeplake_vector_store_paths_for_user,
# )
from src.backend.io import delete_files, save_files
from src.backend.logging import logger
from src.backend.models import MODELS, MODES
from src.streamlit.constants import (
    ACTIVELOOP_HELP,
    AUTHENTICATION_HELP,
    CHUNK_OVERLAP_PCT,
    CHUNK_SIZE,
...
def update_chain() -> None:
    ...
    # except Exception as e:
    #     msg = f"Failed to build chain for data source with model: {e}"
    #     logger.error(msg)
